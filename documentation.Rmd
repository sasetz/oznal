...
We also fit a linear SVM model. Since it is sensitive to scaling, we add normalization and remove correlated numeric predictors. The recipe includes a log transform for the outcome, matching the setup in the script.
This model is fast to fit and gives a useful linear baseline after stronger preprocessing.
...
The prediction scatter helps us see how tight the fit is across the range of salaries.
...
The residuals plot is included to check whether there is a clear pattern left in the errors.

# Forward Selection

We use a forward stepwise procedure on a log-transformed linear model. We bake the recipe into a training frame first, then use `step()` to select predictors.
The selection criterion is set to BIC here to keep the model compact.
...
The scatter plot below summarizes the test-set fit after selection.

```{r forward_plot, echo=FALSE}
...
We fit a lasso regression with the same preprocessing steps as forward selection. The penalty is kept fixed.
This lets us shrink coefficients while keeping the recipe identical for comparison.
...
We show both the prediction scatter and the residuals to match the linear regression section.
...
The ridge model uses the same recipe as lasso, but with an $\ell_2$ penalty.
This keeps all predictors while still regularizing the coefficients.
...
The plots below mirror the lasso outputs for a direct visual comparison.

```{r ridge_plot, echo=FALSE}
